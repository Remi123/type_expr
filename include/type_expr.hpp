...
template <typename... Fs>
struct construct_fs : pipe_<Fs..., call_f> {
  template <typename... Ts>
...
static_assert(pipe_t<input<int, float, int[2], float[2]>,
...
struct sort {
  ...
  struct sort_impl<input<F, Ts...>> {
    typedef typename sort_impl<typename flat<
        input<>, typename eager<Ts, typename Predicate::template f<Ts, F>::
                                        type>::type...>::type>::type Less;
    typedef typename sort_impl<typename flat<
        input<>, typename eager<Ts, typename not_<Predicate>::template f<
